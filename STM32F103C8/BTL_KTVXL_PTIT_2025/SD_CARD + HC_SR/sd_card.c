...
static FATFS g_myFATFS;

static char g_sdPath[4] = "0:/";
static char g_logFile[10];
  ...
  if (response_debug == 0x01) { 
    ...
    do {
      SD_SendCmd(CMD55, 0, 0x65);
			response_debug = SD_SendCmd(ACMD41, 0x40000000, 0x77); // Bit HCS (Host Capacity Support)
			SD_CS_HIGH();
			SD_SPI_SendByte(0xFF);
			i++;
    ...
    do {
      SD_SendCmd(CMD55, 0, 0x65); 
			response_debug = SD_SendCmd(ACMD41, 0x00000000, 0xFF); // Gui ACMD41 (arg=0)
      SD_CS_HIGH(); SD_SPI_SendByte(0xFF);
      i++;
    } while ((response_debug != 0x00) && (i < 0xFFFF));
...
void SD_InitFileSystem(const char *nameFile){
	FRESULT res;
	char uart_buff[64];
	FIL tempFile; // File t?m ch? d? t?o/xóa

	strncpy(g_logFile, nameFile, sizeof(g_logFile) - 1);
	g_logFile[sizeof(g_logFile) - 1] = '\0';
	
    USART_SendString("Dang goi f_mount...\r\n");
    
    // 1. MOUNT (Dùng bi?n toàn c?c g_myFATFS)
    ...
    if (res == FR_OK) {
        ...
        while(1); // L?i nghiêm tr?ng, d?ng ? dây
    }

    // 2. T?O/XÓA FILE LOG (Nhu b?n yêu c?u)
    // Dùng c? FA_CREATE_ALWAYS d? XÓA file log cu t? l?n ch?y tru?c
    ...
    // Ðóng file l?i ngay. 
    // Gi? chúng ta có 1 file "NEW.txt" r?ng, s?n sàng d? ghi ti?p.
    f_close(&tempFile);
    USART_SendString("File log da san sang.\r\n");
}

/**
 * @brief HÀM 2: M? (ghi ti?p) -> Ghi -> Ðóng
 */
void SD_Write_Distance(float distance_cm) {
    ...
    // 1. M? FILE (ÐÃ S?A C?)
    // FA_OPEN_APPEND: M? file và nh?y d?n V? TRÍ CU?I CÙNG (d? ghi ti?p)
    // FA_OPEN_ALWAYS: N?u file b? xóa (do ai dó rút th?), t? t?o l?i
    res = f_open(&fil, g_logFile, FA_OPEN_ALWAYS | FA_WRITE | FA_OPEN_APPEND);
    
    if (res == FR_OK) {
        // 2. CHU?N B? D? LI?U GHI
        ...
        // 4. ÐÓNG FILE (R?t quan tr?ng, f_close s? luu file)
        res = f_close(&fil);
        if (res != FR_OK) {
        ...
        // Ch? thông báo n?u không có l?i ghi ho?c dóng
        if (res == FR_OK) {
